...
version_added: "0.1.0"
description:
  - Create, update, and delete firewall rule categories on an OPNsense firewall via the REST API.
...
options:
  state:
    description: Desired state of the firewall category.
    type: str
    choices: [present, absent]
    default: present
  uuid:
    description:
      - UUID of an existing firewall category.
      - When omitted, the module searches by I(name) to locate an existing category.
    type: str
    required: false
  name:
    description: Category name. Used as natural key.
    type: str
    required: false
  auto:
    description: Whether this is an auto-generated category.
    type: str
    required: false
  color:
    description: Hex color code for this category (e.g. ff0000).
    type: str
    required: false
  reconfigure:
    description: Apply firewall changes immediately after mutating the category.
    type: bool
    default: false
  host:
...
- name: Create a firewall category
  opnsense.core.opnsense_firewall_category:
    host: "{{ opnsense_host }}"
...
RETURN = r"""
firewall_category:
  description: The firewall category object after the operation.
  type: dict
...
from opnsense_py.models.firewall import FirewallCategory

_FIELDS = ("name", "auto", "color")


def _find_by_uuid(client: OPNsenseClient, uuid: str):
    try:
        obj = client.firewall.get_category(uuid)
...
def _find_by_name(client: OPNsenseClient, name: str):
    result = client.firewall.search_categories(SearchRequest(searchPhrase=name))
...
def _build_obj(params: dict) -> FirewallCategory:
    return FirewallCategory(
...
def run(params: dict, check_mode: bool, client: OPNsenseClient) -> dict:
    ...
    if state == "absent":
        ...
        if not check_mode:
            client.firewall.del_category(existing_uuid)
    ...
    if existing_uuid is None:
        if check_mode:
            return {"changed": True, "firewall_category": None}
        obj = _build_obj(params)
        response = client.firewall.add_category(obj)
        new_uuid = response.uuid
        if params.get("reconfigure"):
            client.firewall.apply()
        result = client.firewall.get_category(new_uuid).model_dump(exclude_none=True)
        result["uuid"] = new_uuid
        return {"changed": True, "firewall_category": result}

    if not _needs_update(existing, params):
        existing["uuid"] = existing_uuid
        return {"changed": False, "firewall_category": existing}

    if check_mode:
        return {"changed": True, "firewall_category": existing}

    obj = _build_obj(params)
    client.firewall.set_category(existing_uuid, obj)
    if params.get("reconfigure"):
        client.firewall.apply()
    result = client.firewall.get_category(existing_uuid).model_dump(exclude_none=True)
...
def run_module() -> None:
    argument_spec = dict(
        ...
        auto=dict(type="str", required=False, default=None),
