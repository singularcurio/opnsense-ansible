...
short_description: Manage IPsec remote authentication definitions
version_added: "0.1.0"
description:
  - Create, update, and delete IPsec remote authentication definitions on an OPNsense firewall.
  - Natural key is the I(description) field.
  - Set I(reconfigure=true) to apply changes immediately.
options:
  state:
    description: Desired state of the IPsec remote auth definition.
    type: str
    choices: [present, absent]
    default: present
  uuid:
    description: UUID of an existing IPsec remote auth definition.
    type: str
    required: false
  description:
  ...
  auth:
    ...
    required: false
  id:
    description: Remote IKE identity.
    type: str
    required: false
  eap_id:
    ...
    required: false
  groups:
    description: EAP groups for authentication.
    type: str
    required: false
  certs:
    ...
    required: false
  cacerts:
    description: CA certificate UUIDs for trust chain.
    type: str
    required: false
  pubkeys:
...
EXAMPLES = r"""
- name: Create an IPsec remote auth definition
  opnsense.core.opnsense_ipsec_remote:
    host: "{{ opnsense_host }}"
...
RETURN = r"""
ipsec_remote:
  description: The IPsec remote auth definition object after the operation.
  type: dict
...
from opnsense_py.models.ipsec import IPsecRemote

_FIELDS = (
    "connection", "round", "auth", "id", "eap_id", "groups",
    "certs", "cacerts", "pubkeys", "description",
)
...
def _find_by_uuid(client: OPNsenseClient, uuid: str):
    try:
        obj = client.ipsec.get_remote(uuid)
...
def _find_by_description(client: OPNsenseClient, description: str):
    result = client.ipsec.search_remotes(SearchRequest(searchPhrase=description))
...
def _build_obj(params: dict) -> IPsecRemote:
...
def run(params: dict, check_mode: bool, client: OPNsenseClient) -> dict:
    ...
    if state == "absent":
        ...
        if not check_mode:
            client.ipsec.del_remote(existing_uuid)
    ...
    if existing_uuid is None:
        if check_mode:
            return {"changed": True, "ipsec_remote": None}
        obj = _build_obj(params)
        response = client.ipsec.add_remote(obj)
        new_uuid = response.uuid
        if params.get("reconfigure"):
            client.ipsec.reconfigure()
        result = client.ipsec.get_remote(new_uuid).model_dump(exclude_none=True)
        result["uuid"] = new_uuid
        return {"changed": True, "ipsec_remote": result}

    if not _needs_update(existing, params):
        existing["uuid"] = existing_uuid
        return {"changed": False, "ipsec_remote": existing}

    if check_mode:
        return {"changed": True, "ipsec_remote": existing}

    obj = _build_obj(params)
    client.ipsec.set_remote(existing_uuid, obj)
    if params.get("reconfigure"):
        client.ipsec.reconfigure()
    result = client.ipsec.get_remote(existing_uuid).model_dump(exclude_none=True)
...
def run_module() -> None:
    argument_spec = dict(
        ...
        groups=dict(type="str", required=False, default=None),
        certs=dict(type="str", required=False, default=None),
        cacerts=dict(type="str", required=False, default=None),
